...
public class ChatUtil {
    ...
    public void chatEvent(PacketPlayerChat packet) {
        ...
        boolean eme = rank.getRankId() >= Rank.EARNINGMYEARS.getRankId();
        if (eme) {
            if (player.isAFK()) {
                player.setAFK(false);
    ...
    public void sendChat(Player player, String msg) {
        ...
        String server = Dashboard.getServer(player.getServer()).getServerType();
        String sname = server;
        if (server.startsWith("New")) {
            sname = server.replaceAll("New", "");
        }
        if (Dashboard.getServer(server).isPark()) {
            Rank rank = player.getRank();
    ...
    public void socialSpyMessage(Player from, Player to, String message, String command) {
        if (Dashboard.getServer(from.getServer()).isPark()) {
            ...
            for (Player tp : Dashboard.getOnlinePlayers()) {
                if (tp.getRank().getRankId() < Rank.EARNINGMYEARS.getRankId() || tp.getServer() == null ||
